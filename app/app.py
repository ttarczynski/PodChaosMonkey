...
def delete_random_pod(namespace, label_selector, num_pods):
    """
...
def schedule_pod_deletions(namespace, label_selector, num_pods, schedule):
    logger.info(f'Starting pod deletion schedule with: schedule="{schedule}", namespace="{namespace}", num_pods="{num_pods}", label_selector="{label_selector}".')
    scheduler = BlockingScheduler()
    scheduler.add_job(delete_random_pod, args=[namespace, label_selector, num_pods], trigger=CronTrigger.from_crontab(schedule))
    scheduler.start()
...
if __name__ == '__main__':
    ...
    num_pods = 2
    schedule = '* * * * *'
    schedule_pod_deletions(namespace, label_selector, num_pods, schedule)
